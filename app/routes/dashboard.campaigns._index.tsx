...
import CampaignGoogleAdsJsonRepository from "~/repositories/campaign-googleAds-json.repository";

export async function loader() {
  const repository = new CampaignGoogleAdsJsonRepository();
        ...
        <div className="flex flex-col md:flex-row items-center justify-center sm:justify-between">
          <div>
            <div className="flex items-center gap-x-3">
              <h2 className="text-lg font-medium text-gray-800">Campañas</h2>
              <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full">{data.length} campañas</span>
            </div>
          </div>
          ...
          <GraphSkeleton comprobation={data.length > 0}>
            <ResponsiveContainer width={"100%"} height={320}>
              <LineChart data={data}>
                ...
                <YAxis />
                <CartesianGrid />
                <Line type={"monotone"} dataKey={"convertions"} stroke={"#0ea5e9"} />
                  ...
                  <tbody className="bg-white divide-y divide-gray-200  ">
                    {data.map((item) => (
                      <tr key={item.name}>
                        ...
                        <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                          <div className="inline px-3 py-1 text-sm font-normal rounded-full text-violet-500 gap-x-2 bg-violet-100/60 ">
                            {item.clicks}
                          </div>
                        </td>
                        <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                        ...
                        <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                          <div className="inline px-3 py-1 text-sm font-normal rounded-full gap-x-2 ">${item.cost}</div>
                        </td>

...
import LeadCrmJsonRepository from "~/repositories/lead-crm-json.repository";

export async function loader() {
  const repository = new LeadCrmJsonRepository();
        ...
        <div className="flex flex-col md:flex-row items-center justify-center sm:justify-between">
          <div>
            <div className="flex items-center gap-x-3">
              ...
              <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full">{data.length} leads</span>
            </div>
          </div>
        ...
        <div className="border border-zinc-100 p-2 md:p-5 min-h-80 flex justify-center items-center">
          <GraphSkeleton comprobation={data.length > 0}>
            <ResponsiveContainer width={"100%"} height={320}>
              <LineChart
                data={data.map((item) => ({
                  "Costo de adquisición": item.acquisitionPrice,
                  "Valor de vida": item.lifeValue,
                  nombre: item.name,
                }))}
              >
                <Tooltip />
                ...
                <YAxis />
                <CartesianGrid />
                <Line type={"monotone"} dataKey={"Costo de adquisición"} stroke={"#0ea5e9"} />
                <Line type={"monotone"} dataKey={"Valor de vida"} stroke={"#8b5cf6"} />
              </LineChart>
            </ResponsiveContainer>
          </GraphSkeleton>
        </div>
